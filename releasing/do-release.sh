...
$PREFIX docker buildx build --platform linux/amd64,linux/s390x,linux/arm64,linux/ppc64le --tag dtomcej-netskope/grpcurl:${VERSION} --tag dtomcej-netskope/grpcurl:latest --push --progress plain --no-cache .
$PREFIX docker buildx build --platform linux/amd64,linux/s390x,linux/arm64,linux/ppc64le --tag dtomcej-netskope/grpcurl:${VERSION}-alpine --tag dtomcej-netskope/grpcurl:latest-alpine --push --progress plain --no-cache --target alpine .
rm VERSION

# Homebrew release

URL="https://github.com/dtomcej-netskope/grpcurl/archive/refs/tags/${VERSION}.tar.gz"
